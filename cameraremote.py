...
class CameraRemote(wx.Frame):
    ...
    def discover(self, event):
        asyncio.create_task(self.__camera_remote_control.discover())
    ...
    def __init_menu_bar(self):
        ...
        file_menu = wx.Menu()
        discover_item = file_menu.Append(wx.ID_ANY, "Discover", "Discover Sony camera")
        self.Bind(wx.EVT_MENU, self.discover, discover_item)
    ...
    def __init_ui(self):
        self.SetTitle("Remote control for Sony Cameras")
        self.SetSize(1030, 800)
        self.Centre()
...
camera_remote.Show()
app.SetTopWindow(camera_remote)
...
try:
    sys.exit(loop.run_until_complete(app.MainLoop()))
finally:
    loop.close()
